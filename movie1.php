...
<head>
    ...
    <style>
        body {
            background-image: url("images/background/newbg.jpg");
        }
    </style>
</head>
<center>
    ...
    <div style="color: red; text-align: center;">
        ...
        $Date = date('Y-m-d');
        $Cinema = $_POST['Cinema'];
        $result = mysqli_query($conn, "SELECT ticketnum FROM cinema1");
            ...
            if ($submit) {
                if (strlen($cname) >= 4) {
                    if (strlen($cname) <= 16) {
                        ...
                        mysqli_query($conn, "UPDATE  cinema1 SET  ticketnum =  '$number' WHERE  `cinema1`.`No` =1;");
                        die("<button type='button' onclick=''><A href='receipt.php'> <img src='images/pics/print.jpg' alt='Klematis' /><br />Print Voucher</A></button>");
                    } else {
